    ...
    <div class="product-detail">
        ...
        <form class="product-detail__form" action="{{ route('products.update', $product->id) }}" method="post"
            ...
            <div class="product-detail__top">
                <div class="product-detail__image-area">
                    <div class="product-detail__image-wrapper">
                        <img class="product-detail__image" id="preview-image"
            ...
            <div class="form__group form__group--description">
                ...
                <textarea class="form__textarea" id="description" name="description" rows="8">{{ $product->description }}</textarea>
            </div>
        ...
        <form class="product-detail__delete-form" action="{{ route('products.destroy', $product->id) }}" method="POST"
            onsubmit="return confirm('本当に削除しますか？');">
            @csrf
            @method('DELETE')

            <button type="submit" class="button button--delete" aria-label="削除">
                <svg class="delete-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960"
                    fill="currentColor">
                    <path
